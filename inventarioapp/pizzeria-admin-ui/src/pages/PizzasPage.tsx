    ...
    return (
        <div className="pizzas-page">
            <div className="page-header">
                <h2>Pizzas</h2>
                {!showCreateForm && (
                    <button onClick={() => setShowCreateForm(true)} className="button-add">
            ...
            {showCreateForm && (
                <div className="create-pizza-form-container">
                    <CrearPizzaForm /> 
                     <button onClick={() => setShowCreateForm(false)} style={{marginTop: '10px'}}>Cancelar</button>
                </div>
            )}

            {error && <p className="error-message">{error}</p>}
            {loading && <p>Cargando pizzas...</p>}
            {!loading && pizzas.length === 0 && !showCreateForm && (
                <p>No hay pizzas en el menú. ¡Añade la primera!</p>
            )}

            {!showCreateForm && pizzas.length > 0 && (
                    ...
                    {pizzas.map((pizza) => (
                        <div key={pizza.menuItemId} className="pizza-card">
                            ...
                            <div className="pizza-card-actions">
                                ...
                                <button className="action-button delete-button" onClick={() => handleDeletePizza(pizza.menuItemId)}>🗑️</button>
                            </div>
                        </div>
                    ))}
                </div>
